...
function Dashboard() {
  ...
  const setDashboardSubtitle = (car) => {
    ...
    const { engineDisplacement, maker, model, transmission, variant, year } =
      car;
    return `${maker} ${model} ${year} ${
      +engineDisplacement / 1000
    }L ${variant} ${transmission}`;
  };

  return (
    <ScrollView style={styles.appContainer}>
      {openConfig && (
        <CarDetailsForm open={openConfig} setOpen={setOpenConfig} />
      )}
      {openAddFuelRecord && (
        <AddFuelEfficiencyRecord
          open={openAddFuelRecord}
          setOpen={setOpenAddFuelRecord}
        />
      )}
      {openAddExpensesRecord && (
        <AddExpensesRecord
          open={openAddExpensesRecord}
          setOpen={setOpenAddExpensesRecord}
        />
      )}
